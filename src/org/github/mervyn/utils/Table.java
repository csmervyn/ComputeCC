...
public class Table {
	...
	public static int maxElementInTable(Table table){
		long max = 0;
		for(int lineNum = 0; lineNum < table.getLineList().size(); lineNum++){
			...
			for(int groupNum = 0; groupNum < maxGroupNum; groupNum++){
				...
				for(int columnNum = 0; columnNum < maxColumnNum; columnNum++){
					long temp = table.getLineList().get(lineNum).getGroupList().get(groupNum).getColumnList().get(columnNum);
					if(temp > max){
						max = temp;
					}
				}
	...
	public static int minElementInTable(Table table){
