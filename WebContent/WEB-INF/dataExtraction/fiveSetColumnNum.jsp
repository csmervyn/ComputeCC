...
<body class="container">
	<h3>数据抽取-设置每组的列数</h3>
	<form action="<%=basePath%>DEC/fiveDataExtraction.do" method="post" class="form-horizontal">
		...
		<c:forEach  begin="0" end="${groupNum-1}" varStatus="varStatus">
			<c:choose>
					...
					<c:when test="${varStatus.index== 24}">
						<div class="form-group">
							<label for="column${varStatus.index}" class="col-sm-2 control-label">第Y组：</label>
							<div class="col-sm-3">
								<input type="text" class="form-control" id="column${varStatus.index}" name="column${varStatus.index}">
							</div>
						</div>
